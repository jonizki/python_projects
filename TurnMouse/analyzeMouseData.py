...
mouse_Folder = "C:\\Users\\Omistaja\\Desktop\\MouseRepeater-main\\TurnMouse\\mousemovements\\"
...
for i in range(amount_Of_Files):
    ...
    print("mouse movement length: " + str(length))
    ...
    with open(mouse_Movement_Folder + str(length) + "\\" + str(degree) + "," + str(A) + "," + str(B) + "," + str(zone) + ".txt", "w") as file:
        for i in range(len(x_Coords)):
            if i < len(x_Coords)-1:
                file.write(str(x_Coords[i]) + "," + str(y_Coords[i]) + "," + "0.0" + str(int(movement_ms[i+1])-int(movement_ms[i])) + "\n")
            else:
                file.write(str(x_Coords[i]) + "," + str(y_Coords[i]) + "," + "0.0" + str(int(movement_ms[i])-int(movement_ms[i-1])))
